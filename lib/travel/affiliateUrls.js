...
export function buildAffiliateUrls(dest, { budget, season, duration, adults = 2, children = 0, childAge = null }) {
  ...
  const bookingUrl = `https://www.booking.com/searchresults.de.html?ss=${encodeURIComponent(searchCity)}&checkin=${ci}&checkout=${co}&group_adults=${adults}&group_children=${children}&no_rooms=1${bookingAgeParam}&order=${BOOKING_ORDER[apiBudget] ?? 'popularity'}&lang=de`;
  const skyUrl = iata
