 ...
 * Filenames are always auto-generated from type + slug — the original filename is ignored.
 *
 * @param {File} file
 * @param {string} slug          destination slug (used as folder name)
 * @param {'hero'|'og'|'twitter'|'gallery'} type
 * @param {number} [galleryIndex=0]  0-based position for gallery images (→ gallery-01, gallery-02 …)
 * @returns {{ path: string, url: string, filename: string }}
 */
export async function uploadDestinationImage(file, slug, type, galleryIndex = 0) {
  ...
  else if (type === 'og')      filename = `open-graph.${ext}`;
...
/**
 * Upload a File for a blog article.
 * Uses the same bucket as destinations but under the blog/ prefix.
 *
 * @param {File} file
 * @param {string} slug            blog article slug
 * @param {'cover'|'hero'|'gallery'} type
 ...
 *   blog/{slug}/cover.jpg
 *   blog/{slug}/hero.jpg
 *   blog/{slug}/gallery-01.jpg
 */
export async function uploadBlogImage(file, slug, type, galleryIndex = 0) {
  const supabase = getStorageClient();
