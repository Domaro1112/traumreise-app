...
// check24_hotel / check24_urlaub / check24_mietwagen / check24_car_rental are aliases for check24 (same pid param, same settings entry)
// AWIN-Anbieter alle via awinaffid — URL wird als AWIN-Deeplink gebaut
export const AFFILIATE_PARAMS = {
  ...
  check24_hotel:      'pid',
  check24_urlaub:     'pid',
  check24_mietwagen:  'pid',
  check24_car_rental: 'pid',
  holidaycheck:       'ref',
  amazon:            'tag',
  expedia:           'affcid',
  trivago:           'ptId',
...
export const ALLOWED_BASE_DOMAINS = [
  ...
  // AWIN tracking domain (alle AWIN-Deeplinks laufen über awin1.com)
  'awin1.com',
  // Provider-Domains als Direktlinks-Fallback
  'centerparcs.de',
  'landal.de', 'landalcamping.de',
  'roompot.de',
  'topparken.de', 'topparken.com',
  'sunparks.de',
  'eurocamp.de',
  'novasol.de',
];
...
export async function generateAffiliateUrl(provider, targetUrl) {
  ...
  try {
    ...
    // check24_hotel / check24_urlaub / check24_mietwagen use the same affiliate_settings entry as check24
    const settingsKey = provider.startsWith('check24_') ? 'check24' : provider;

    const settings = await getCachedAffiliateSettings();
    const setting = settings[settingsKey];

    if (!setting) {
      console.warn(`[affiliate] Kein Eintrag in affiliate_settings für Provider "${settingsKey}" (angefragt: "${provider}"). Migration ausgeführt?`);
      return targetUrl;
    }
    if (!setting.enabled) {
