...
Thema: (Platzhalter) Pflichtfelder (camelCase, alle Texte auf Deutsch):
...
* relatedDestinations (Array mit 2–3 thematisch verwandten Slug-Namen) Beginne jeden contentSection mit einem direkten 40–60-Wörter-Antwortsatz. Wichtig für keyTakeaways:
...
LLMO-Anforderung: Der Artikel muss so strukturiert sein, dass KI-Systeme die wichtigsten Erkenntnisse, Empfehlungen und Antworten leicht extrahieren können.
Nutze klare Zwischenüberschriften, konkrete Aussagen, Listen und präzise Antworten. Nenne zu jeder Sehenswürdigkeit und jedem Transport-Hinweis mindestens eine verifizierbare, stabile Kennzahl: Entfernung, Fahrt-/Zugzeit, Eintritt oder Preis-Range mit Stand-Datum, Sterne/Kategorie oder Öffnungszeit.
Keine erfundenen Werte — ist etwas nicht sicher belegbar, setze [prüfen] statt zu schätzen. Bei sicherheitsrelevanten Zielen aktuelle offizielle Reisehinweise (z. B. Auswärtiges Amt) berücksichtigen. 
Liegt eine Reisewarnung oder ein Abraten vor, dies prominent und kalibriert kennzeichnen (eigener Abschnitt + FAQ), nicht werblich darstellen; Status immer mit Stand-Datum. Keine Reisewarnung erfinden — nur belegte. Gib ausschließlich gültiges JSON zurück, ohne Markdown-Umrandung, ohne Erklärung.`;
