...
export async function generateMetadata({ params }) {
  ...
  try {
    ...
    const { data } = await supabase
      ...
      .eq('slug', slug).eq('status', 'published').eq('type', 'guide').single();
    if (!data) return { title: 'Reiseguide | ApeAround' };
    const creator = data.creator_profiles?.display_name ?? 'ApeAround';
    return {
      title: `${data.title} | ApeAround Reiseguide`,
      description: data.excerpt || `Reiseguide von ${creator} auf ApeAround.`,
      alternates: { canonical: `/reiseguides/${slug}` },
      openGraph: { title: data.title, description: data.excerpt, type: 'article' },
    };
  } catch {
    return { title: 'Reiseguide | ApeAround' };
  }
}

export default async function ReiseguideDetailPage({ params }) {
  const { slug } = await params;
  let guide = null;
  try {
    const supabase = createServerClient();
  ...
  const creator = guide.creator_profiles;
  ...
  return (
    <>
      ...
      <main style={{ minHeight: '100vh', background: '#FFFFFF' }}>
        ...
        <section style={{
          ...
          <div style={{ maxWidth: '780px', margin: '0 auto' }}>
            ...
            <div style={{ display: 'flex', gap: '8px', flexWrap: 'wrap', marginBottom: '14px' }}>
              <span style={{ padding: '3px 12px', borderRadius: '20px', background: 'rgba(124,58,237,0.3)', color: '#C4B5FD', fontSize: '12px', fontWeight: 600 }}>📖 Reiseguide</span>
              {guide.destination && <span style={{ padding: '3px 12px', borderRadius: '20px', background: 'rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.85)', fontSize: '12px' }}>📍 {guide.destination}</span>}
              {guide.country    && <span style={{ padding: '3px 12px', borderRadius: '20px', background: 'rgba(255,255,255,0.12)', color: 'rgba(255,255,255,0.85)', fontSize: '12px' }}>{guide.country}</span>}
            </div>
            ...
            {guide.excerpt && (
              <p style={{ fontSize: 'clamp(15px, 2vw, 17px)', color: 'rgba(255,255,255,0.82)', lineHeight: 1.7, margin: 0, maxWidth: '600px' }}>
                {guide.excerpt}
              </p>
            )}
            {creator && (
        ...
        {/* Content */}
        <article style={{ maxWidth: '780px', margin: '0 auto', padding: 'clamp(36px, 6vw, 56px) clamp(20px, 5vw, 40px)' }}>
          {tags.length > 0 && (
            <div style={{ display: 'flex', flexWrap: 'wrap', gap: '8px', marginBottom: '28px' }}>
              {tags.map(t => (
                <span key={t} style={{ padding: '4px 12px', borderRadius: '20px', background: '#F5F3FF', color: '#7C3AED', fontSize: '12px', fontWeight: 500 }}>{t}</span>
              ))}
            </div>
          )}
          {guide.content && (
            <div style={{ fontSize: '16px', color: '#1E293B', lineHeight: 1.85, whiteSpace: 'pre-wrap' }}>
              {guide.content}
            </div>
          )}
        </article>
        ...
        {images.length > 1 && (
          <section style={{ background: '#F8FAFC', padding: 'clamp(32px, 5vw, 48px) 0' }}>
            <div style={{ maxWidth: '900px', margin: '0 auto', padding: '0 clamp(20px, 5vw, 40px)' }}>
              <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: '20px', fontWeight: 800, color: '#0F172A', margin: '0 0 18px', letterSpacing: '-0.02em' }}>Bilder</h2>
        ...
        {/* Creator box */}
        {creator && (
          <section style={{ maxWidth: '780px', margin: '0 auto', padding: '0 clamp(20px, 5vw, 40px) clamp(40px, 6vw, 56px)' }}>
            <div style={{ background: '#F8FAFC', border: '1.5px solid #E2E8F0', borderRadius: '16px', padding: '22px 24px', display: 'flex', alignItems: 'center', gap: '16px', flexWrap: 'wrap' }}>
              ...
              <div style={{ flex: 1, minWidth: '160px' }}>
                <p style={{ margin: '0 0 2px', fontSize: '11px', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em' }}>Creator</p>
        ...
        <section style={{ background: 'linear-gradient(135deg, #0F172A 0%, #1E3A5F 100%)', padding: 'clamp(44px, 7vw, 64px) clamp(20px, 5vw, 40px)', textAlign: 'center' }}>
          <div style={{ maxWidth: '520px', margin: '0 auto' }}>
            <h2 style={{ fontFamily: 'var(--font-heading)', fontSize: 'clamp(18px, 3vw, 24px)', fontWeight: 800, color: '#FFFFFF', margin: '0 0 12px', letterSpacing: '-0.02em' }}>
              Lass dir eine passende Reise vorschlagen
            </h2>
            <p style={{ fontSize: '15px', color: 'rgba(255,255,255,0.7)', lineHeight: 1.7, margin: '0 0 24px' }}>
              Finde deine nächste Traumreise mit ApeAround.
            </p>
