    ...
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.8)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "linear-gradient(160deg,#0d1220,#07070f)", border: "1px solid rgba(255,215,0,.2)", borderRadius: "24px", padding: "36px 32px", maxWidth: "420px", width: "100%", position: "relative" }}>
        ...
        <input type="email" value={email} onChange={e => setEmail(e.target.value)} placeholder="deine@email.de"
          style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,.06)", border: `1.5px solid ${email.includes("@") ? "rgba(255,215,0,.4)" : "rgba(255,255,255,.12)"}`, borderRadius: "12px", padding: "13px 16px", color: "#fff", fontSize: "15px", outline: "none", marginBottom: "14px" }} />
        ...
        <button onClick={() => valid && setDone(true)} disabled={!valid}
          style={{ width: "100%", padding: "14px", borderRadius: "12px", border: "none", background: valid ? "linear-gradient(90deg,#FFD700,#FF8C00)" : "rgba(255,255,255,.08)", color: valid ? "#0a0a14" : "rgba(255,255,255,.2)", fontWeight: 700, fontSize: "15px", cursor: valid ? "pointer" : "not-allowed" }}>
          📬 Kostenlos anmelden
        </button>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px", maxWidth: "560px", margin: "0 auto" }}>
        {[
          ...
          <button key={m.id} onClick={() => onSelect(m.id)}
            style={{ padding: "24px 20px", borderRadius: "20px", textAlign: "left", border: `1.5px solid ${m.color}33`, background: "rgba(255,255,255,.03)", cursor: "pointer", transition: "all .25s", position: "relative" }}
...
function Classic({ onBack }) {
  ...
  const [departure, setDeparture] = useState(""); // Abflughafen IATA
  ...
  // ── Fallback-Daten aus Saison wenn keine konkreten Daten gewählt ──
  const getDefaultDates = () => {
    const now    = new Date();
    const year   = now.getFullYear();
    const nights = { weekend: 3, week: 7, twoweeks: 14, long: 21 }[duration] || 7;
    const seasonStart = {
      spring: new Date(year, 3, 15),   // 15. April
      summer: new Date(year, 6, 10),   // 10. Juli
      autumn: new Date(year, 9, 10),   // 10. Oktober
      winter: new Date(year, 11, 20),  // 20. Dezember
  ...
  // ── Budget → Nächte → Preis pro Nacht berechnen ──
  const calcPricePerNight = () => {
    const nights           = { weekend: 3, week: 7, twoweeks: 14, long: 21 }[duration] || 7;
    const totalPerPerson   = { low: 400, mid: 1000, high: 2500 }[budget] || 1000;
    const hotelBudgetTotal = totalPerPerson * adults * 0.4;
    const pricePerNight    = Math.round(hotelBudgetTotal / nights);
    return {
      price_max: budget === "high" ? null    : pricePerNight,
      price_min: budget === "high" ? 150     : budget === "mid" ? 50 : null,
      order:     budget === "low"  ? "price" : budget === "high" ? "class_asc" : null,
    };
  };
  ...
  const buildAffiliateUrls = (d) => {
    ...
    // ── Trivago — direkt Hotelpreisvergleich ──
    // Trivago zeigt sofort Ergebnisse ohne Suchfeld
    ...
    // ── Booking.com — nur zuverlässige Parameter ──
  ...
  const fetch_ = async () => {
    ...
    try {
      ...
      setResults(parsed.destinations.map(d => ({
        ...d,
        ...buildAffiliateUrls(d),
      })));
      setStep(4);
    ...
    <>
      ...
      {step === 0 && (
        <div style={{ animation: "fadeUp .55s ease both" }}>
          ...
          <div style={{ display: "flex", flexWrap: "wrap", gap: "8px", marginBottom: "14px" }}>
            {EXAMPLES.map((ex, i) => (
              <button key={i} onClick={() => setFreeText(ex)} style={{ padding: "6px 14px", borderRadius: "20px", fontSize: "12px", border: "1px solid rgba(255,215,0,.2)", background: "rgba(255,215,0,.05)", color: "rgba(255,255,255,.5)", cursor: "pointer" }}
          ...
          <div style={{ position: "relative" }}>
            <textarea value={freeText} onChange={e => setFreeText(e.target.value)} rows={5} placeholder="Ich träume von einem Ort, wo morgens die Gassen noch leer sind…"
              style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,.04)", border: `1.5px solid ${freeText.length >= 20 ? "rgba(255,215,0,.45)" : "rgba(255,255,255,.1)"}`, borderRadius: "14px", padding: "16px 18px 36px", color: "#fff", fontSize: "15px", lineHeight: 1.75, fontFamily: "'Lato',sans-serif", fontWeight: 300, resize: "none", outline: "none" }} />
          ...
          {[{ label: "👤 Erwachsene", sub: "18+ Jahre", val: adults, set: setAdults, min: 1 }, { label: "🧒 Kinder", sub: "unter 18 Jahre", val: children, set: setChildren, min: 0 }].map(({ label, sub, val, set, min }) => (
            <div key={label} style={{ display: "flex", alignItems: "center", justifyContent: "space-between", background: "rgba(255,255,255,.04)", border: "1px solid rgba(255,255,255,.08)", borderRadius: "16px", padding: "20px 24px", marginBottom: "14px" }}>
              ...
              <div style={{ display: "flex", alignItems: "center", gap: "16px" }}>
                <button onClick={() => set(v => Math.max(min, v - 1))} style={{ width: "36px", height: "36px", borderRadius: "50%", border: "1.5px solid rgba(255,215,0,.4)", background: "transparent", color: "#FFD700", fontSize: "20px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>−</button>
                <span style={{ fontSize: "22px", fontWeight: 700, minWidth: "28px", textAlign: "center" }}>{val}</span>
                <button onClick={() => set(v => v + 1)} style={{ width: "36px", height: "36px", borderRadius: "50%", border: "1.5px solid rgba(255,215,0,.4)", background: "rgba(255,215,0,.1)", color: "#FFD700", fontSize: "20px", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>+</button>
              </div>
            </div>
          ))}
          <div style={{ marginTop: "16px" }}>
            <div style={{ fontSize: "13px", color: "#FFD700", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "12px", fontWeight: 600 }}>🗓️ Reisedaten <span style={{ color: "rgba(255,255,255,.3)", fontSize: "11px", textTransform: "none", fontWeight: 400, letterSpacing: 0 }}>(optional — sonst Saison-Schätzung)</span></div>
              ...
              {[{ label: "Anreise", val: checkin, set: setCheckin }, { label: "Abreise", val: checkout, set: setCheckout }].map(({ label, val, set }) => (
                <div key={label}>
                  ...
                  <input type="date" value={val} min={new Date().toISOString().split("T")[0]} onChange={e => set(e.target.value)}
                    style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,.05)", border: `1.5px solid ${val ? "rgba(255,215,0,.4)" : "rgba(255,255,255,.1)"}`, borderRadius: "10px", padding: "10px 14px", color: val ? "#fff" : "rgba(255,255,255,.3)", fontSize: "14px", outline: "none", colorScheme: "dark" }} />
                </div>
              ))}
            </div>
            {checkin && checkout && <div style={{ marginTop: "8px", fontSize: "13px", color: "rgba(255,215,0,.7)" }}>✓ Daten werden direkt übertragen</div>}
          </div>

          {/* Abflughafen */}
          <div style={{ marginTop: "22px" }}>
            <div style={{ fontSize: "13px", color: "#FFD700", letterSpacing: "2px", textTransform: "uppercase", marginBottom: "12px", fontWeight: 600 }}>
              ✈️ Dein Abflughafen <span style={{ color: "rgba(255,255,255,.3)", fontSize: "11px", textTransform: "none", fontWeight: 400, letterSpacing: 0 }}>(für direkte Flugsuche)</span>
            </div>
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(120px,1fr))", gap: "8px" }}>
              {[
                { code: "FRA", label: "🏙️ Frankfurt" },
                ...
                <button key={ap.code} onClick={() => setDeparture(d => d === ap.code ? "" : ap.code)}
                  style={{ padding: "10px 6px", borderRadius: "10px", border: `1.5px solid ${departure === ap.code ? "#FFD700" : "rgba(255,255,255,.1)"}`, background: departure === ap.code ? "rgba(255,215,0,.12)" : "rgba(255,255,255,.03)", color: departure === ap.code ? "#FFD700" : "rgba(255,255,255,.6)", cursor: "pointer", fontSize: "12px", fontWeight: departure === ap.code ? 700 : 400, transition: "all .2s", textAlign: "center", lineHeight: 1.5 }}>
                  {ap.label}<br/><span style={{ fontSize: "11px", opacity: .65 }}>{ap.code}</span>
                </button>
              ))}
            </div>
            {departure
              ? <div style={{ marginTop: "10px", fontSize: "13px", color: "rgba(255,215,0,.7)" }}>✓ Skyscanner zeigt direkte Flüge ab {departure}</div>
              : <div style={{ marginTop: "8px", fontSize: "12px", color: "rgba(255,255,255,.25)", fontStyle: "italic" }}>Ohne Auswahl: allgemeine Flugsuche zum Zielort</div>
            }
          </div>
        </div>
      )}
        ...
        <div style={{ animation: "fadeIn .5s ease" }}>
          {personality && (
            <div style={{ marginBottom: "32px", background: "linear-gradient(135deg,rgba(255,215,0,.08),rgba(255,100,80,.06))", border: "1px solid rgba(255,215,0,.25)", borderRadius: "20px", padding: "26px 28px", position: "relative", overflow: "hidden" }}>
              <div style={{ position: "absolute", top: "-20px", right: "-20px", fontSize: "110px", opacity: .04, lineHeight: 1 }}>🧭</div>
          ...
          <div style={{ background: "linear-gradient(135deg,rgba(255,215,0,.08),rgba(167,139,250,.06))", border: "1px solid rgba(255,215,0,.2)", borderRadius: "16px", padding: "20px 24px", marginBottom: "20px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "16px", flexWrap: "wrap" }}>
            ...
            <button onClick={() => setShowEmail(true)} style={{ padding: "11px 22px", borderRadius: "10px", border: "none", background: "linear-gradient(90deg,#FFD700,#FF8C00)", color: "#0a0a14", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}>Kostenlos →</button>
          </div>
          ...
          <div style={{ textAlign: "center" }}>
            <button onClick={reset} style={{ padding: "11px 26px", borderRadius: "10px", border: "1.5px solid rgba(255,255,255,.18)", background: "transparent", color: "rgba(255,255,255,.55)", cursor: "pointer", fontSize: "14px" }}
      ...
      {step < 4 && !loading && (
        <div style={{ display: "flex", justifyContent: "space-between", marginTop: "28px" }}>
          <button onClick={() => step === 0 ? onBack() : setStep(s => s - 1)} style={{ padding: "13px 24px", borderRadius: "10px", border: "1.5px solid rgba(255,255,255,.14)", background: "transparent", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: "15px", fontWeight: 500 }}>
            ← {step === 0 ? "Zurück" : "Zurück"}
          </button>
          <button onClick={() => step < 3 ? setStep(s => s + 1) : fetch_()} disabled={!canGo()}
            style={{ padding: "13px 32px", borderRadius: "10px", border: "none", fontSize: "15px", fontWeight: 700, background: canGo() ? "linear-gradient(90deg,#FFD700,#FF8C00)" : "rgba(255,255,255,.08)", color: canGo() ? "#0a0a14" : "rgba(255,255,255,.2)", cursor: canGo() ? "pointer" : "not-allowed", boxShadow: canGo() ? "0 4px 20px rgba(255,180,0,.3)" : "none" }}>
        ...
        <div style={{ animation: "fadeUp .55s ease .15s both" }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "repeat(4,1fr)", gap: "10px", marginBottom: "24px" }}>
            {VIBES.map(v => {
              ...
              return (
                <button key={v.id} onClick={() => toggle(v.id)} style={{ padding: "14px 8px", borderRadius: "16px", border: `1.5px solid ${sel ? v.color : "rgba(255,255,255,.1)"}`, background: sel ? `${v.color}18` : "rgba(255,255,255,.03)", cursor: "pointer", transition: "all .25s", display: "flex", flexDirection: "column", alignItems: "center", gap: "5px", boxShadow: sel ? `0 0 18px ${v.color}44` : "none" }}>
          ...
          <textarea value={text} onChange={e => setText(e.target.value)} rows={3} placeholder="Ich brauche Abstand… oder ich will endlich wieder richtig leben…"
            style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,.04)", border: "1.5px solid rgba(255,255,255,.08)", borderRadius: "14px", padding: "14px 18px", color: "#fff", fontSize: "14px", lineHeight: 1.7, fontFamily: "'Lato',sans-serif", fontWeight: 300, resize: "none", outline: "none", marginBottom: "24px" }}
          ...
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            <button onClick={onBack} style={{ padding: "13px 24px", borderRadius: "10px", border: "1.5px solid rgba(255,255,255,.14)", background: "transparent", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: "15px", fontWeight: 500 }}>← Zurück</button>
            <button onClick={fetch_} disabled={vibes.length === 0}
              style={{ padding: "16px 40px", borderRadius: "14px", border: "none", fontSize: "16px", fontWeight: 700, background: vibes.length > 0 ? "linear-gradient(90deg,#A78BFA,#7C3AED)" : "rgba(255,255,255,.08)", color: vibes.length > 0 ? "#fff" : "rgba(255,255,255,.2)", cursor: vibes.length > 0 ? "pointer" : "not-allowed", boxShadow: vibes.length > 0 ? "0 6px 28px rgba(167,139,250,.4)" : "none", animation: vibes.length > 0 ? "pulse 2s ease infinite" : "none" }}>
        ...
        <div style={{ animation: "fadeIn .5s ease" }}>
          ...
          {showBtns && (
            <div style={{ animation: "fadeUp .5s ease" }}>
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px", marginBottom: "14px" }}>
                <button onClick={next} style={{ padding: "13px", borderRadius: "12px", border: `1.5px solid ${color}44`, background: `${color}10`, color, fontWeight: 700, fontSize: "14px", cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background = `${color}22`} onMouseLeave={e => e.currentTarget.style.background = `${color}10`}>😈 Alternatives Leben</button>
                <button onClick={() => setShowShare(true)} style={{ padding: "13px", borderRadius: "12px", border: "1.5px solid rgba(255,255,255,.15)", background: "rgba(255,255,255,.05)", color: "#fff", fontWeight: 700, fontSize: "14px", cursor: "pointer" }} onMouseEnter={e => e.currentTarget.style.background = "rgba(255,255,255,.1)"} onMouseLeave={e => e.currentTarget.style.background = "rgba(255,255,255,.05)"}>📲 Reise-Ich teilen</button>
              </div>
              <div style={{ textAlign: "center", marginBottom: "16px" }}>
                <button onClick={reset} style={{ background: "none", border: "none", color: "rgba(255,255,255,.3)", fontSize: "13px", cursor: "pointer", textDecoration: "underline" }}>↩ Neu starten</button>
              </div>
              <div style={{ background: "linear-gradient(135deg,rgba(255,215,0,.08),rgba(167,139,250,.06))", border: "1px solid rgba(255,215,0,.2)", borderRadius: "16px", padding: "18px 22px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: "14px", flexWrap: "wrap" }}>
                <div><div style={{ fontSize: "14px", fontWeight: 600, color: "#fff", marginBottom: "3px" }}>📬 Reise-Ich per Mail sichern</div><div style={{ fontSize: "12px", color: "rgba(255,255,255,.4)" }}>+ wöchentlich neue Deals</div></div>
                <button onClick={() => setShowEmail(true)} style={{ padding: "10px 20px", borderRadius: "10px", border: "none", background: "linear-gradient(90deg,#FFD700,#FF8C00)", color: "#0a0a14", fontWeight: 700, fontSize: "13px", cursor: "pointer" }}>Kostenlos →</button>
              </div>
              <div style={{ textAlign: "center", fontSize: "11px", color: "rgba(255,255,255,.18)", marginTop: "14px" }}>* Affiliate-Links — keine Mehrkosten für dich 🙏</div>
            </div>
          )}
        </div>
      )}
      ...
      {showShare && cur && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
          <div style={{ background: "linear-gradient(135deg,#0d0d1a,#1a1a2e)", border: `1.5px solid ${color}44`, borderRadius: "24px", padding: "40px 32px", maxWidth: "380px", width: "100%", textAlign: "center", position: "relative" }}>
            ...
            <div style={{ fontSize: 11, color, letterSpacing: 3, textTransform: "uppercase", marginBottom: 6 }}>Dein Reise-Ich</div>
            ...
            <button onClick={() => { const t = `✈️ Mein Reise-Ich: ${cur.destination}\n„${cur.teaser}"\n\nFinde dein Reise-Ich → traumreise.ai`; if (navigator.share) navigator.share({ text: t }); else navigator.clipboard?.writeText(t); }} style={{ width: "100%", padding: "13px", borderRadius: 12, border: "none", background: `linear-gradient(90deg,${color},${color}bb)`, color: "#000", fontWeight: 700, fontSize: 15, cursor: "pointer" }}>📲 Teile dein Reise-Ich</button>
          </div>
        </div>
      )}
...
// ─── BACKGROUND IMAGES ───────────────────────────────────────────────────────
// Ersetze diese URLs mit deinen echten Bild-URLs auf dem Server
const BG_IMAGES = [
  "https://images.unsplash.com/photo-1506905925346-21bda4d32df4?w=1600&q=80", // Berge & Meer Sonnenuntergang
  "https://images.unsplash.com/photo-1519046904884-53103b34b206?w=1600&q=80", // Küste Sonnenuntergang
  "https://images.unsplash.com/photo-1439405326854-014607f694d7?w=1600&q=80", // Holzterrasse Meerblick
];

const BG_LABELS = ["Bild 1 — Bergküste", "Bild 2 — Bucht", "Bild 3 — Terrasse"];

// ─── ADMIN BG SWITCHER ────────────────────────────────────────────────────────
function BgSwitcher({ current, onChange, onClose }) {
  const [pw, setPw] = useState("");
  const [open, setOpen] = useState(false);

  if (!open) return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.85)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "#0d1220", border: "1px solid rgba(255,215,0,.2)", borderRadius: "20px", padding: "36px", maxWidth: "340px", width: "100%", textAlign: "center" }}>
        <div style={{ fontSize: "28px", marginBottom: "12px" }}>🖼️</div>
        <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: "20px", color: "#fff", marginBottom: "6px" }}>Hintergrundbild wechseln</h3>
        <p style={{ color: "rgba(255,255,255,.4)", fontSize: "13px", marginBottom: "20px" }}>Admin-Passwort eingeben</p>
        <input type="password" value={pw} onChange={e => setPw(e.target.value)} onKeyDown={e => e.key === "Enter" && pw === "reise2024" && setOpen(true)} placeholder="Passwort…"
          style={{ width: "100%", boxSizing: "border-box", background: "rgba(255,255,255,.06)", border: "1.5px solid rgba(255,255,255,.15)", borderRadius: "10px", padding: "11px 14px", color: "#fff", fontSize: "15px", outline: "none", marginBottom: "12px", textAlign: "center" }} />
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px" }}>
          <button onClick={onClose} style={{ padding: "11px", borderRadius: "10px", border: "1.5px solid rgba(255,255,255,.15)", background: "transparent", color: "rgba(255,255,255,.5)", cursor: "pointer", fontSize: "14px" }}>Abbrechen</button>
          <button onClick={() => pw === "reise2024" ? setOpen(true) : alert("Falsches Passwort")} style={{ padding: "11px", borderRadius: "10px", border: "none", background: "linear-gradient(90deg,#FFD700,#FF8C00)", color: "#0a0a14", fontWeight: 700, fontSize: "14px", cursor: "pointer" }}>Weiter</button>
        </div>
      </div>
    </div>
  );
  ...
  return (
    <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,.9)", zIndex: 400, display: "flex", alignItems: "center", justifyContent: "center", padding: "20px" }}>
      <div style={{ background: "#0d1220", border: "1px solid rgba(255,215,0,.2)", borderRadius: "24px", padding: "32px", maxWidth: "620px", width: "100%" }}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: "24px" }}>
          <div>
            <div style={{ fontSize: "11px", color: "#FFD700", letterSpacing: "3px", textTransform: "uppercase", marginBottom: "4px" }}>Admin</div>
            <h3 style={{ fontFamily: "'Playfair Display',serif", fontSize: "22px", color: "#fff", margin: 0 }}>🖼️ Hintergrundbild wählen</h3>
          </div>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(3,1fr)", gap: "12px", marginBottom: "20px" }}>
          {BG_IMAGES.map((url, i) => (
            <div key={i} onClick={() => onChange(i)}
              style={{ borderRadius: "12px", overflow: "hidden", cursor: "pointer", border: `2.5px solid ${current === i ? "#FFD700" : "rgba(255,255,255,.1)"}`, transition: "all .2s", boxShadow: current === i ? "0 0 20px rgba(255,215,0,.4)" : "none", position: "relative" }}>
              <img src={url} alt={BG_LABELS[i]} style={{ width: "100%", height: "110px", objectFit: "cover", display: "block" }} />
              <div style={{ padding: "8px 10px", background: "rgba(0,0,0,.6)" }}>
                <div style={{ fontSize: "11px", color: current === i ? "#FFD700" : "rgba(255,255,255,.6)", fontWeight: current === i ? 700 : 400 }}>{BG_LABELS[i]}</div>
              </div>
              {current === i && (
                <div style={{ position: "absolute", top: 8, right: 8, width: 22, height: 22, borderRadius: "50%", background: "#FFD700", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#000" }}>✓</div>
              )}
            </div>
          ))}
        </div>

        <div style={{ padding: "14px 16px", background: "rgba(255,215,0,.06)", border: "1px solid rgba(255,215,0,.15)", borderRadius: "12px", fontSize: "13px", color: "rgba(255,255,255,.5)", marginBottom: "16px" }}>
          💡 <strong style={{ color: "#FFD700" }}>Tipp:</strong> Ersetze die Bild-URLs in <code style={{ color: "#A78BFA" }}>BG_IMAGES</code> im Code mit deinen eigenen Bildern von deinem Server.
        </div>

        <button onClick={onClose} style={{ width: "100%", padding: "13px", borderRadius: "12px", border: "none", background: "linear-gradient(90deg,#FFD700,#FF8C00)", color: "#0a0a14", fontWeight: 700, fontSize: "15px", cursor: "pointer" }}>
          ✓ Speichern & Schließen
        </button>
      </div>
    </div>
...
export default function App() {
  ...
  const [bgIdx, setBgIdx] = useState(() => {
    try { return parseInt(localStorage.getItem("traumreise_bg") || "0"); } catch { return 0; }
  });
  const [showBgAdmin, setShowBgAdmin] = useState(false);

  const handleBgChange = (i) => {
    setBgIdx(i);
    try { localStorage.setItem("traumreise_bg", String(i)); } catch {}
  };

  return (
    <div style={{ minHeight: "100vh", fontFamily: "'Lato',sans-serif", color: "#fff", overflowX: "hidden", position: "relative" }}>
      ...
      {/* ── Hero Background Image (only on Home page) ── */}
      {page === "home" && (
        <>
          <div key={bgIdx} style={{
            ...
            animation: "bgFade .8s ease",
          }} />
          {/* Dark gradient overlay so text stays readable */}
          <div style={{
            position: "fixed", inset: 0, zIndex: 1,
            background: "linear-gradient(to bottom, rgba(7,7,15,.55) 0%, rgba(7,7,15,.7) 50%, rgba(7,7,15,.9) 100%)",
          }} />
        </>
      )}
      ...
      {page !== "home" && (
        <div style={{ position: "fixed", inset: 0, zIndex: 0, background: "linear-gradient(160deg,#07070f 0%,#0d1220 50%,#070f07 100%)" }} />
      )}

      {/* Stars (only on inner pages) */}
      {page !== "home" && (
      ...
      {/* Page content */}
      <div style={{ position: "relative", zIndex: 2, maxWidth: "820px", margin: "0 auto", padding: "40px 20px 80px" }}>
      ...
      <div style={{ position: "fixed", bottom: "16px", right: "16px", zIndex: 50, display: "flex", gap: "8px" }}>
        {/* Bg switcher button */}
        <button onClick={() => setShowBgAdmin(true)} title="Hintergrundbild wechseln"
          style={{ width: "40px", height: "40px", borderRadius: "50%", border: "1px solid rgba(255,255,255,.1)", background: "rgba(255,255,255,.05)", color: "rgba(255,255,255,.3)", fontSize: "16px", cursor: "pointer", transition: "all .2s", backdropFilter: "blur(8px)" }}
          onMouseEnter={e => { e.currentTarget.style.borderColor = "rgba(255,215,0,.4)"; e.currentTarget.style.color = "#FFD700"; e.currentTarget.style.background = "rgba(255,215,0,.08)"; }}
          onMouseLeave={e => { e.currentTarget.style.borderColor = "rgba(255,255,255,.1)"; e.currentTarget.style.color = "rgba(255,255,255,.3)"; e.currentTarget.style.background = "rgba(255,255,255,.05)"; }}>
          🖼️
        </button>
      </div>

      {/* Modals */}
      {showBgAdmin && <BgSwitcher current={bgIdx} onChange={handleBgChange} onClose={() => setShowBgAdmin(false)} />}
    </div>
  );
}
