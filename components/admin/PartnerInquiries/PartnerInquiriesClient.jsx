...
const STATUS_LABELS = { new: 'Neu', contacted: 'Kontaktiert', closed: 'Abgeschlossen' };
const STATUS_COLORS = {
  new:       { bg: 'rgba(14,165,233,0.10)', color: '#0EA5E9' },
  contacted: { bg: 'rgba(234,179,8,0.12)',  color: '#CA8A04' },
  closed:    { bg: 'rgba(100,116,139,0.12)', color: '#475569' },
};

export default function PartnerInquiriesClient({ initialData }) {
  ...
  if (!inquiries.length) {
    return (
      <div style={{
        ...
        Noch keine Partner-Anfragen eingegangen.
      </div>
    );
  }
      ...
      {inquiries.map(item => {
        ...
        return (
          <div key={item.id} style={{
            ...
            <div style={{
              ...
              <div style={{ flex: 1, minWidth: 0 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: '10px', flexWrap: 'wrap' }}>
                  ...
                  {item.company && (
                    <span style={{ fontSize: '12px', color: '#64748B' }}>· {item.company}</span>
                  )}
                  {item.partner_type && (
                    <span style={{
                      fontSize: '11px', fontWeight: 600, padding: '2px 8px',
                      borderRadius: '20px', background: 'rgba(14,165,233,0.08)', color: '#0EA5E9',
                    }}>{item.partner_type}</span>
                  )}
                  <span style={{
                    fontSize: '11px', fontWeight: 600, padding: '2px 8px',
            ...
            {isOpen && (
              <div style={{
                borderTop: '1px solid #F1F5F9', padding: '16px 18px 18px',
                background: '#FAFBFC',
              }}>
                {item.message && (
                ...
                {item.cooperation_type && (
                  <div style={{ marginBottom: '16px' }}>
                    <div style={{ fontSize: '11px', fontWeight: 700, color: '#94A3B8', textTransform: 'uppercase', letterSpacing: '0.06em', marginBottom: '4px' }}>Kooperation</div>
                    <div style={{ fontSize: '13px', color: '#374151' }}>{item.cooperation_type}</div>
                  </div>
                )}
